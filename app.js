...
class QuizApp {
    ...
    async startQuiz(dataFile, fileName, questionsToStudy = null, shouldShuffle = false) {
        ...
        if (questionsToStudy) {
            ...
            this.questions = shouldShuffle ? this.shuffleArray([...questionsToStudy]) : [...questionsToStudy];
        } else {
    ...
    async loadQuestions(dataFile, shouldShuffle = false) {
        try {
            ...
            document.getElementById('question-text').textContent = 'Không thể tải câu hỏi. Vui lòng kiểm tra lại dữ liệu.';
        }
    }
    ...
    renderQuestion() {
        ...
        if (question.type === 'fill_in_blank') {
            ...
            if (this.evaluatedQuestions[this.currentQuestionIndex]) {
                input.disabled = true;
                if (hasCorrectAnswer) {
                    const isCorrect = this.checkFillAnswer(input.value, question.correctAnswer);
            ...
            question.options.forEach((option, index) => {
                ...
                const isSelected = isMultiple
                    ? Array.isArray(this.selectedAnswer) && this.selectedAnswer.includes(index)
                    : this.selectedAnswer !== null && this.selectedAnswer === index;
                if (isSelected) {
                ...
                if (isEvaluated) {
                    ...
                    if (isMultiple) {
                        ...
                        if (isSelected && index !== question.correctAnswerIndex) {
                            button.classList.add('incorrect');
                        }
                    }
                    button.disabled = true;
                } else {
                    // Not evaluated yet: allow user interaction
                    button.addEventListener('click', () => this.handleOptionClick(index, button));
                }
        ...
        if (this.currentQuestionIndex === this.questions.length - 1) {
            ...
            if (question.type === 'fill_in_blank' && !this.evaluatedQuestions[this.currentQuestionIndex]) {
    ...
    evaluateSingleChoice(answerIndex, button) {
        ...
        if (isCorrect) {
            ...
            if (question.correctAnswerIndex >= 0 && question.correctAnswerIndex < allButtons.length) {
                allButtons[question.correctAnswerIndex].classList.add('correct');
            }
        }
    ...
    updateResultMessage() {
        ...
        if (isFillInBlank) {
            ...
            // Display correct answer indices (+1 for human-readable)
            correctAnswerDisplay = correctIndices.map(i => i + 1).join(', ');
        } else {
            // Single choice
            isCorrect = this.selectedAnswer === question.correctAnswerIndex;
            ...
            if (idx >= 0 && question.options && question.options[idx]) {
                correctAnswerDisplay = `${idx + 1}. ${question.options[idx]}`;
            } else if (idx >= 0) {
    ...
    showResults() {
        ...
        const studyIncorrectBtns = document.querySelectorAll('.study-incorrect-btn');
    ...
    restartAll(shouldShuffle) {
        ...
        this.startQuiz(this.currentDataFile, this.currentFileName, null, shouldShuffle);
    }

    restartIncorrect(shouldShuffle) {
        if (this.incorrectQuestions && this.incorrectQuestions.length > 0) {
            document.getElementById('results-screen').style.display = 'none';
            this.startQuiz(null, this.currentFileName, this.incorrectQuestions, shouldShuffle);
        }
    }
